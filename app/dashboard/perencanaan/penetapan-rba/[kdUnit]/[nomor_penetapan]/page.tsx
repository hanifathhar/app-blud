...
import { ArrowLeft, Building, Calendar, FileText, ChevronDown, ChevronRight } from "lucide-react";
import { useRouter } from "next/navigation";

export default function PenetapanDetailPage({ params }: { params: Promise<{ kdUnit: string, nomor_penetapan: string }> }) {
                  ...
                  <React.Fragment key={rbaIdx}>
                    ...
                    >
                      ...
                      <td colSpan={2} style={{ padding: "12px 16px", verticalAlign: "top" }}>
                        ...
                        <div style={{ fontSize: 11, color: "#64748B", marginTop: 4, fontWeight: 600 }}>
                          SUMBER DANA: <span style={{ color: "#3B82F6" }}>{rbaHeader.sumdan || "-"}</span> | NO RBA: {rbaHeader.no_rba}
                        </div>
                      </td>
                      <td style={{ padding: "12px 16px", fontSize: 13, fontWeight: 800, color: "#0F172A", textAlign: "right", verticalAlign: "top" }}>
                        {new Intl.NumberFormat('id-ID').format(rbaHeader.nilai || 0)}
                      </td>
                    </tr>
                    ...
                    {!isCollapsed && (
                      rbaHeader.rincian && rbaHeader.rincian.length > 0 ? (
                        rbaHeader.rincian.map((rin: any, rinIdx: number) => (
                          <tr key={`${rbaIdx}-${rinIdx}`} style={{ borderBottom: "1px solid #E2E8F0", backgroundColor: "#FFFFFF" }} className="hover:bg-slate-50/50">
                            <td style={{ padding: "12px 16px", fontSize: 12, color: "#475569", textAlign: "center", verticalAlign: "top" }}>
                              {rin.kd_rek6}
                            </td>
                            ...
                            <td style={{ padding: "12px 16px", fontSize: 12, verticalAlign: "top", paddingLeft: 0 }}>
                              <div style={{ fontWeight: 600, color: "#1E293B" }}>{rin.nm_rek6}</div>
                              {rin.uraian && <div style={{ color: "#475569", marginTop: 4 }}>- {rin.uraian}</div>}
