...
export default function PukPage() {
  ...
  const loadData = () => {
    ...
    fetch(`/api/perencanaan/puk?${query.toString()}`)
      .then(r => r.json())
      .then(d => {
        setList(d.data || []);
  ...
  const handleDelete = async (id: string) => {
    const result = await Swal.fire({
      title: "Hapus PUK?",
      text: "Data PUK akan dihapus secara permanen.",
      icon: "warning",
    ...
    try {
      const res = await fetch(`/api/perencanaan/puk/${id}`, { method: "DELETE" });
      ...
      Swal.fire({
        icon: "success",
        title: "Terhapus",
        text: "Data PUK berhasil dihapus.",
        timer: 1500,
  ...
  const handleEdit = (item: PUK) => {
    router.push(`/dashboard/perencanaan/puk/${item.id}/rincian`);
  };
  ...
  const handlePrint = () => {
    ...
    const url = new URL("/dashboard/perencanaan/puk/cetak", window.location.origin);
  ...
  return (
    <div className="animate-fadein relative">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0F172A" }}>📑 Rencana Usulan Kegiatan Belanja</h1>
        </div>
        <div style={{ display: "flex", gap: 12 }}>
          ...
          >
            <FileText size={16} /> Cetak PUK
          </button>
          <Link href="/dashboard/perencanaan/puk/create">
            <button className="btn btn-primary">
              <Plus size={16} /> Tambah PUK
            </button>
          </Link>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title">Daftar PUK</span>
        </div>
          ...
          <>
            ...
            {list.length === 0 ? (
              <div className="empty-state" style={{ padding: 48, borderTop: "1px solid #E2E8F0" }}>
                ...
                <p style={{ fontWeight: 600 }}>Belum ada data PUK</p>
              </div>
            ) : (
      ...
      {showPrintModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#FFF", borderRadius: 8, padding: 24, width: 400, maxWidth: "90%" }}>
            <h3 style={{ margin: "0 0 16px 0", fontSize: 18, color: "#0F172A" }}>Cetak PUK</h3>
