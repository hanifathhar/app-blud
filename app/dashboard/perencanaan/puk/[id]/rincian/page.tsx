...
export default function PukRincianPage() {
  ...
  const loadData = async () => {
    ...
    try {
      // 1. Fetch Master PUK
      const resPuk = await fetch(`/api/perencanaan/puk/${id}`);
      ...
      if (dataPuk.data?.noPuk) {
        const resRincian = await fetch(`/api/perencanaan/puk/rincian?noPuk=${encodeURIComponent(dataPuk.data.noPuk)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const url = form.id ? `/api/perencanaan/puk/rincian/${form.id}` : "/api/perencanaan/puk/rincian";
  ...
  const handleInfoSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/perencanaan/puk/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (rincianId: number) => {
    ...
    try {
      await fetch(`/api/perencanaan/puk/rincian/${rincianId}`, { method: "DELETE" });
      loadData();
    } catch (e) {
      ...
      <div className="card mb-10" style={{ padding: 0, overflow: "hidden", background: "#fff", border: "1px solid #E2E8F0", boxShadow: "0 4px 6px -1px rgba(0, 0, 0, 0.05)" }}>
        ...
        <div style={{ padding: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "220px 10px 1fr", gap: "8px 0", fontSize: 13, color: "#334155", marginBottom: 24 }}>
            ...
            <div>:</div>
            <div>{puk.kdUkm ? `${puk.kdUkm} - ${puk.nmUkm}` : puk.nmUkm || "-"}</div>

            <div>Peruntukan</div>
            <div>:</div>
            <div>{puk.kdPeruntukan ? `${puk.kdPeruntukan} - ${puk.nmPeruntukan}` : puk.nmPeruntukan || "-"}</div>

            <div>Komponen</div>
            <div>:</div>
            <div>{puk.kdKomponen ? `${puk.kdKomponen} - ${puk.nmKomponen}` : puk.nmKomponen || "-"}</div>

            <div>Sub Komponen</div>
            <div>:</div>
            <div>{puk.kdRincian ? `${puk.kdRincian} - ${puk.nmRincian}` : puk.nmRincian || "-"}</div>
            ...
            <div>:</div>
            <div>{puk.kdSpm ? `${puk.kdSpm} - ${puk.nmSpm}` : puk.nmSpm || "-"}</div>
          </div>
              ...
              <table className="tbl">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style={{ fontSize: 14, color: '#1E293B' }}>{puk.sumdan || "-"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div style={{
            ...
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #F1F5F9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: "#0F172A", margin: 0 }}>
                {form.id ? "📝 Edit Rincian PUK" : "➕ Tambah Rincian PUK"}
              </h2>
              ...
              <div style={{ padding: 24, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <Select menuPosition="fixed" options={[
                    ...
                    ...sumdans.map(sd => ({ value: sd.sumdan, label: sd.sumdan }))
                  ]}
                    value={{ value: infoForm.sumdan, label: infoForm.sumdan || "-- Pilih Sumber Dana --" }}
                    onChange={(e: any) => setInfoForm({ ...infoForm, sumdan: e.value })}
                    styles={{
                      control: (base) => ({ ...base, fontSize: 13, background: "#F8FAFC", borderColor: "#E2E8F0" })
                    }}
                  />
                </div>
              </div>
