...
export default function RincianRbaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = () => {
    setLoading(true);
    fetch(`/api/perencanaan/rba-pendapatan/${no_rba}/rincian`)
      .then(r => r.json())
      .then(d => {
  ...
  const loadRek6Options = async (inputValue: string) => {
    const res = await fetch(`/api/master/rek6?q=${inputValue}&limit=1000&startsWith=4`);
  ...
  const openAddModal = async () => {
    try {
      const res = await fetch(`/api/perencanaan/rba-pendapatan/${no_rba}/puk-rincian`);
      const d = await res.json();
      if (d.data) {
  ...
  const saveAdd = async () => {
    ...
    setSavingAdd(true);
    try {
      const res = await fetch(`/api/perencanaan/rba-pendapatan/${no_rba}/rincian`, {
        method: "POST",
      ...
      Swal.fire({ icon: "success", title: "Berhasil!", text: "Rincian berhasil ditambahkan.", timer: 1500, showConfirmButton: false });
      setShowAddModal(false);
      loadData();
    } catch (e: any) {
      Swal.fire({ icon: "error", title: "Gagal", text: e.message || "Gagal menyimpan rincian" });
    } finally {
      setSavingAdd(false);
    }
  };
  ...
  const saveEdit = async () => {
    if (!editForm.kd_rek6) {
      Swal.fire({
        icon: "warning",
        title: "Peringatan",
        text: "Kode Rekening Pendapatan harus dipilih sebelum menyimpan.",
  ...
  const deleteRincian = async (item: any) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await fetch(`/api/perencanaan/rba-pendapatan/${no_rba}/rincian?rincian_id=${item.id}`, {
          method: "DELETE",
        });
        const d = await res.json();
        if (!res.ok) throw new Error(d.error);
  ...
  const handleBack = async () => {
    ...
    if (hasEmptyRekening) {
      const result = await Swal.fire({
        ...
        text: "Masih ada rincian yang belum memiliki Kode Rekening Pendapatan. Jika Anda tetap kembali, seluruh data RBA ini akan dibatalkan/dihapus. Lanjutkan?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#EF4444",
        cancelButtonColor: "#64748B",
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`/api/perencanaan/rba-pendapatan/${no_rba}`, { method: "DELETE" });
          ...
          router.push("/dashboard/perencanaan/rba-pendapatan");
        } catch (e: any) {
          Swal.fire({
            icon: "error",
            title: "Error",
  ...
  return (
    <div className="animate-fadein relative pb-10">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0F172A" }}>Rincian RBA Pendapatan</h1>
        </div>
      ...
      {showEditModal && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "#FFF", borderRadius: 8, padding: 24, width: 600, maxWidth: "100%", boxShadow: "0 10px 25px rgba(0,0,0,0.1)" }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#475569", marginBottom: 6 }}>Kode Rekening Pendapatan</label>
              <AsyncSelect
                cacheOptions
                defaultOptions
                ...
                placeholder="Pilih Rekening Pendapatan..."
              />
            </div>
