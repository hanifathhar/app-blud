...
export default function RbaPage() {
  ...
  const loadData = () => {
    ...
    fetch(`/api/perencanaan/rba-pendapatan?${query.toString()}`)
      .then(r => r.json())
      .then(d => {
        setList(d.data || []);
  ...
  const loadPukOptions = async () => {
    try {
      const res = await fetch("/api/perencanaan/rba-pendapatan/puk-options");
  ...
  const handleCreateRba = async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch("/api/perencanaan/rba-pendapatan/copy-from-puk", {
        method: "POST",
      ...
      setShowModal(false);
      loadData();
      router.push(`/dashboard/perencanaan/rba-pendapatan/${data.rba.no_rba}/rincian`);
    } catch (e: any) {
      Swal.fire({
        icon: "error",
        title: "Gagal",
  ...
  const handleEdit = (item: any) => {
    router.push(`/dashboard/perencanaan/rba-pendapatan/${item.no_rba}/rincian`);
  };
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      const res = await fetch(`/api/perencanaan/rba-pendapatan/${item.no_rba}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
  ...
  const handleCetak = () => {
    ...
    window.open(`/dashboard/perencanaan/rba-pendapatan/cetak?${query.toString()}`, '_blank');
  ...
  return (
    <div className="animate-fadein relative">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0F172A" }}>💼 Rencana Bisnis dan Anggaran (RBA - Pendapatan)</h1>
        </div>
                  ...
                  <tbody>
                    {list.map((item, i) => (
                      <tr key={item.id}>
                        ...
                        <td style={{ fontSize: 13, color: "#334155" }}>{item.nmUkm || "NON PROGRAM"}</td>
                        <td style={{ fontSize: 13, color: "#334155" }}>{item.nmKomponen || "PENDAPATAN"}</td>
                        <td style={{ fontSize: 13, color: "#334155", fontWeight: 600 }}>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "#FFF", borderRadius: 8, padding: 24, width: 1000, maxWidth: "100%", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div>
                <h3 style={{ margin: "0 0 4px 0", fontSize: 18, color: "#0F172A", fontWeight: 700 }}>Pilih RUK</h3>
                <p style={{ fontSize: 13, color: "#64748B", margin: 0 }}>Pilih RUK yang belum dijadikan RBA dari daftar di bawah ini.</p>
              </div>
                    ...
                    {upts.filter(u =>
                      ...
                      <tr
                        ...
                        onClick={() => {
                          ...
                          setPukSearch("");
                          fetch(`/api/perencanaan/rba-pendapatan/puk-options?kd_upt=${u.kd_upt}`)
                            .then(r => r.json())
                    ...
                    {filteredPukOptions.length === 0 ? (
                      ...
                      filteredPukOptions.map((p) => (
                        ...
                        >
                          ...
                          <td style={{ fontSize: 13, color: "#334155" }}>{p.nmUkm || "NON PROGRAM"}</td>
                          <td style={{ fontSize: 13, color: "#334155" }}>{p.nmKomponen || "PENDAPATAN"}</td>
