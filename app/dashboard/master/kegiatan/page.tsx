...
export default function MasterKegiatanPage() {
  ...
  const loadPrograms = async () => {
    try {
      const res = await fetch("/api/master/program?limit=1000");
  ...
  const loadData = () => {
    ...
    fetch(`/api/master/kegiatan?${params.toString()}`)
      .then((r) => r.json())
      .then((d) => {
        setList(d.data || []);
  ...
  useEffect(() => { loadData(); }, [page, limit, searchQuery, filterProgram]);

  const openCreate = () => {
    setEditing(null);
    setForm({ kd_kegiatan: "", kd_program: filterProgram || "", nm_kegiatan: "", jns_kegiatan: "", lpermen: "" });
    setShowForm(true);
  };

  const openEdit = (t: Kegiatan) => {
    setEditing(t);
    setForm({
      kd_kegiatan: t.kd_kegiatan || "",
      kd_program: t.kd_program || "",
      nm_kegiatan: t.nm_kegiatan || "",
      jns_kegiatan: t.jns_kegiatan ? t.jns_kegiatan.toString() : "",
      lpermen: t.lpermen || "",
    });
    setShowForm(true);
  };
  ...
  const deleteKegiatan = async (t: Kegiatan) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus Kegiatan ${t.kd_kegiatan}?`)) return;
    try {
      const res = await fetch(`/api/master/kegiatan?id=${t.id}`, { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="animate-fadein">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0F172A" }}>📂 Master Kegiatan</h1>
          <p style={{ fontSize: 13, color: "#64748B", marginTop: 2 }}>Kelola referensi data kegiatan yang terhubung dengan Program BLUD</p>
        </div>
        {!showForm ? (
          <button className="btn btn-primary" onClick={openCreate}><Plus size={16} /> Tambah Kegiatan</button>
        ) : (
        ...
        <>
          ...
          <div className="card">
            <div className="card-header">
              <span className="card-title">Daftar Kegiatan</span>
            </div>
            {loading ? (
              ...
              <div className="empty-state" style={{ padding: 48 }}>
                ...
                <p style={{ fontWeight: 600 }}>Belum ada data kegiatan</p>
              </div>
            ) : (
                ...
                <table className="tbl">
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: 150 }}>Kode Kegiatan</th>
                      <th>Nama Kegiatan</th>
                      <th>Program</th>
                    ...
                    {list.map((t, i) => {
                      ...
                      return (
                        <tr key={t.id}>
                          ...
                          <td style={{ fontWeight: 700, fontSize: 14, color: "#0F172A", fontFamily: "monospace" }}>{t.kd_kegiatan}</td>
                          <td style={{ fontSize: 13, color: "#334155", fontWeight: 500 }}>{t.nm_kegiatan}</td>
                          <td style={{ fontSize: 12, color: "#64748B" }}>
                            <div style={{ fontWeight: 600, color: "#0F172A" }}>{t.kd_program || "-"}</div>
                            <div style={{ fontSize: 11, opacity: 0.8, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 200 }} title={prog?.nm_program}>
                              {prog ? prog.nm_program : "Tidak ditemukan"}
                            </div>
                          </td>
                          <td>
                            <div style={{ display: "flex", gap: 5 }}>
                              <button className="btn btn-outline btn-sm" onClick={() => openEdit(t)} title="Edit Kegiatan">
                                <Edit3 size={12} />
                              </button>
                              <button className="btn btn-sm" style={{ background: "#FEE2E2", color: "#DC2626", borderColor: "#FECACA" }} onClick={() => deleteKegiatan(t)} title="Hapus">
                                <Trash2 size={12} />
                              </button>
                            </div>
                          </td>
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">
              {editing ? `✏️ Edit Kegiatan ${editing.kd_kegiatan}` : "➕ Tambah Kegiatan Baru"}
            </span>
          </div>
          <form onSubmit={handleSubmit}>
            <div className="card-body" style={{ display: "grid", gap: 24, padding: "24px 32px" }}>
              
              {/* Group Relasi Program */}
              <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 12, padding: 20 }}>
                ...
                <div>
                  <label className="form-label">Pilih Program Terkait *</label>
                  <select 
                    className="form-select" 
                    required 
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 20 }}>
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="Misal: 1.01.01.2.01"
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setForm({ ...form, nm_kegiatan: e.target.value })} 
                      placeholder="Misal: Penyediaan Jasa Pelayanan Kesehatan"
                    />
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  <div>
                    ...
                    <input 
                      ...
                      step="0.01" 
                      className="form-input" 
                      value={form.jns_kegiatan} 
                      onChange={(e) => setForm({ ...form, jns_kegiatan: e.target.value })} 
                      placeholder="Opsional (Float)"
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      className="form-input" 
                      value={form.lpermen} 
                      onChange={(e) => setForm({ ...form, lpermen: e.target.value })} 
            ...
            <div className="card-footer" style={{ padding: "16px 24px", borderTop: "1px solid #E2E8F0", display: "flex", justifyContent: "flex-end", gap: 12 }}>
              ...
              <button type="submit" className="btn btn-primary" disabled={submitting}>
                ...
                {submitting ? "Menyimpan..." : editing ? "Simpan Perubahan" : "Simpan Kegiatan"}
              </button>
            </div>
          </form>
        </div>
